...
public class WorldGenWaterTree extends WorldGenAbstractTree {
    private final int minTreeHeight;
    private final int randomTreeHeight;
    private final boolean worldgen;

    public WorldGenWaterTree(boolean doBlockNotify, int minTreeHeight, int randomTreeHeight) {
        super(doBlockNotify);
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (!soil.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, (BlockSapling) TMElementCore.waterSapling)) {
            return false;
        }
            ...
            for (int ix = x - 5; ix <= x + 5; ix++) {
                for (int iz = z - 5; iz <= z + 5; iz++) {
                    ...
                    if (dist < 10 + rand.nextInt(8) &&
                        ...
                        setBlockAndNotify(world, ix, iy, iz, TMElementCore.waterLeaves, 0);
                    }
                }
            }
        }
        ...
        for (int iy = 0; iy < height; iy++) {
            ...
            if (block.isAir(world, x, y + iy, z) ||
                ...
                if (iy > 0 && !lastBlockWasLog && rand.nextInt(chance) == 0) {
                    ...
                    setBlockAndNotify(world, x, y + iy, z, TMElementCore.waterLog, 2);
                    chance += height;
                    lastBlockWasLog = true;
                } else {
                    // Основной ствол
                    setBlockAndNotify(world, x, y + iy, z, TMElementCore.waterLog, 1);
                    lastBlockWasLog = false;
                }

                // Боковые ветки
                setBlockAndNotify(world, x - 1, y + iy, z, TMElementCore.waterLog, 1);
                setBlockAndNotify(world, x + 1, y + iy, z, TMElementCore.waterLog, 1);
                setBlockAndNotify(world, x, y + iy, z - 1, TMElementCore.waterLog, 1);
                setBlockAndNotify(world, x, y + iy, z + 1, TMElementCore.waterLog, 1);
            }
        }

        // Верхушка дерева
        setBlockAndNotify(world, x, y + height, z, TMElementCore.waterLog, 1);

        // Основание с корнями
        setBlockAndNotify(world, x - 1, y, z - 1, TMElementCore.waterLog, 1);
        setBlockAndNotify(world, x + 1, y, z + 1, TMElementCore.waterLog, 1);
        setBlockAndNotify(world, x - 1, y, z + 1, TMElementCore.waterLog, 1);
        setBlockAndNotify(world, x + 1, y, z - 1, TMElementCore.waterLog, 1);

        // Дополнительные ветки у основания
        if (rand.nextInt(3) != 0) setBlockAndNotify(world, x - 1, y + 1, z - 1, TMElementCore.waterLog, 1);
        if (rand.nextInt(3) != 0) setBlockAndNotify(world, x + 1, y + 1, z + 1, TMElementCore.waterLog, 1);
        if (rand.nextInt(3) != 0) setBlockAndNotify(world, x - 1, y + 1, z + 1, TMElementCore.waterLog, 1);
        if (rand.nextInt(3) != 0) setBlockAndNotify(world, x + 1, y + 1, z - 1, TMElementCore.waterLog, 1);

        // Длинные горизонтальные ветки
        setBlockAndNotify(world, x - 2, y, z, TMElementCore.waterLog, 5); // Запад
        setBlockAndNotify(world, x + 2, y, z, TMElementCore.waterLog, 5); // Восток
        setBlockAndNotify(world, x, y, z - 2, TMElementCore.waterLog, 9); // Север
        setBlockAndNotify(world, x, y, z + 2, TMElementCore.waterLog, 9); // Юг
